    ...
    public function index()
    {
        ...
        if (isset($this->request->get['_route_'])) {
            ...
            foreach ($parts as $part) {
				// $this->log->write($this->db->escape($part));
                $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE keyword = '" . $this->db->escape($part) . "' order by url_alias_id asc");
				// $this->log->write($query);
                if ($query->num_rows) {
                    $url = explode('=', $query->row['query']);
					// $this->log->write(count($url));
                    if (count($url) > 1) {
						// $this->log->write($url[0]);
						// $this->log->write($url[1]);
                        if ($url[0] == 'product_id') {
